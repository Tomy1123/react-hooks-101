...
import { timeCurrentTso8601 } from '../utils'
...
const EventForm = () => {
  ...
  const addEvent = e => {
    ...
    dispatch({
      type: ADD_OPERATION_LOG.replace,
      discription: 'イベントを作成しました。',
      operatedAt: timeCurrentTso8601()
    })
  ...
  return (
    ...
    <form>
      ...
      <div className="form-group">
        <label htmlFor="fromEventBody">ボディー</label>
        <textarea className="form-control" id="fromEventBody" value={body} onChange={e => setBody(e.target.value)} />
      </div>

      <button className="btn btn-primary" onClick={addEvent} disabled={unCreatable}>イベントを作成する</button>
      <button className="btn btn-danger" onClick={deleteAllEvents} disabled={state.events.length === 0}>全てを削除する</button>
    </form>
    </>
  )
}
